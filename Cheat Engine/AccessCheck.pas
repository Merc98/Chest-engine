...
begin
  try
    f:=TFilestream.Create(AppDir+'accesscheck.tmp', fmCreate);
    try
  ...
  try
    f:=TFilestream.Create(AppDir+'accesscheck.tmp', fmOpenReadWrite);
    try
      f.Seek(0,soFromEnd);
  ...
  if not deletefile(AppDir+'accesscheck.tmp') then
