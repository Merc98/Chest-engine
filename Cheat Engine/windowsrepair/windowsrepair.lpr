...
begin
  ...
  try
    ...
    begin
      reg.deletekey('VoiceService.exe');
      reg.deletekey('voiceservice-i386.exe');
      reg.deletekey('voiceservice-x86_64.exe');
    end;
  ...
  if (ParamCount=0) or (ParamStr(1)<>'/s') then
    messagebox(0,'Your windows install should be repaired. Try running VoiceService now', 'Windows Repair',0);
end.
